...
skpt.getskilldesc3Phantom = function (skillcode){
    ...
    switch(skillcode){
        case 4:
            ...
            break;   
        case 23:
            rep = "소환수나 설치 스킬이 아닌 팬텀의 공격이 크리티컬로 성공할 때마다 일정 확률로 팬텀으로부터 카드가 나와 주위 적을 자동 공격한다. 이 때 카드 스택의 수치가 1 증가한다.\n일정 시간마다 반전 카드가 준비되어 팬텀의 스킬 중 직접 공격하는 스킬 적중 및 탤런트 오브 팬텀 시프로 사용하는 공격 스킬 적중 시 해당 스킬이 적중하지 않은 주변의 적을 추가로 공격한다. 반전 카드는 카드 스택을 증가시키지 않고 공격 반사 상태의 적을 공격해도 피해를 입지 않는다.\n또한 추가 회피 확률이 영구적으로 증가한다.\n\n팬텀의 공격이 크리티컬로 성공할 때 100%의 확률로 카드가 나와 주위 적을 270%의 데미지로 자동 공격하고 카드 스택 수치 1증가\n반전 카드 : 최대 4명의 적을 900%의 데미지로 공격, 재발동 대기시간 5초\n\n추가 회피 확률 20% 증가\n카드의 일반 몬스터 공격 시 데미지 150% 증가";
            break;     
        case 24:
