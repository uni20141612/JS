...
skmer.getskilldesc1Mercedes = function (skillcode){
    ...
    switch(skillcode){
        ...
        case 19:
            ...
            break;
        case 20:
            rep = "발 당 280% 데미지로 2번/ 이그니스로어 시간 0.1초씩 증가/ 슈스탠";
            break;
        case 21:
            rep = "하단 참조";
            break;
        ...
        case 25:
            ...
            break;
        case 26:
            rep = "최대 8명의 적들에게 456% 데미지의 화살 4개 발사";
            break;
        case 27:
            rep = "적의 방어율 25% 무시, 최종 데미지 20%";
            break;
        case 28:
            rep = "120초 동안 공격력 80 증가";
            break;
        case 29:
            rep = "하단 참조";
            break;
...
skmer.getskilldesc3Mercedes = function (skillcode){
    ...
    switch(skillcode){
        ...
        case 17:
            ...
            break;
        case 18:
            rep = "냉기 정령, 화염 정령, 암흑 정령 중 1명을 소환하여 전투를 돕게 한다. 엘리멘탈 나이트가 활성화된 상태에서 공격 적중 시 정령이 소환된다. 엘리멘탈 나이트는 공격 반사 상태의 적을 공격해도 피해를 입지 않는다.\n스킬 사용 시 효과가 활성화되고 재사용 시 비활성화되는 온오프 스킬\n\nMP 60 소비하여 3명의 정령 중 1명을 210초 동안 소환, 최대 2명까지 소환 가능\n냉기 정령 : 435%의 데미지로 공격하여 대상을 90%확률로 5초 동안 결빙\n화염 정령 : 435%로 공격, 대상은 5초 동안 1초당 110%의 도트 데미지\n암흑 정령 : 548%로 공격";
            break;
        case 19:
            rep = "불의 정령의 힘을 빌어 연계 스킬 사용 시 최종 데미지와 공격 속도를 증가시킨다.\n\n연계 스킬 사용 시 최종 데미지 2%, 공격 속도 1단계 증가\n연계 스킬 사용으로 인한 최종 데미지 증가는 최대 10회까지 중첩, 30초 동안 유지, 중첩되는 최종 데미지 간에는 합적용\n영구적으로 20% 확률로 추가 회피, 최종 데미지 25%, 공격력 40 증가";
            break;
        case 21:
            rep = "도약과 동시에 전설의 창을 떨어트려 전방의 적 다수를 공격한다. 대상의 방어력을 일정수준 감소시키는 효과와 함께 100% 크리티컬 판정으로 적중한다. 스킬 사용 중 적의 어떠한 공격에도 밀려나지 않는다. 추가로 리프 토네이도의 데미지를 영구적으로 증가시키는 패시브 효과가 있다. 리프 토네이도, 거스트 다이브, 롤링 문썰트, 래쓰 오브 엔릴과 연계 가능하며 연계 스킬 사용 시 재사용 대기시간이 1초씩 줄어든다.\n\nMP 60 소비, 최대 12명의 적을 630%로 4번 크리티컬 공격. 30초 동안 대상의 방어력 30% 감소 효과 적용\n재사용 대기시간 5초\n연계 스킬 사용 시 재사용 대기시간 1초 감소\n\n[패시브 효과 : 리프 토네이도 데미지 206%p 증가]";
            break;
        case 22:
            rep = "전방을 향해 순간적으로 이동하며 경로 내 적 다수를 공격한다. 추가로 거스트 다이브의 데미지를 영구적으로 증가시킨다. 차지 드라이브, 스트라이크 듀얼샷, 유니콘 스파이크, 레전드리 스피어, 래쓰 오브 엔릴로 연계 가능하다.\n\nMP 74 소비, 최대 8명의 적을 420%의 데미지로 3번 공격하며 전방으로 이동\n\n[패시브 효과 : 거스트 다이브 데미지 237%p 증가]";
            break;
        case 23:
            rep = "공중에서 빠르게 회전하며 연속 공격한다. 스킬 사용 중 적의 어떠한 공격에도 밀려나지 않는다. 차지 드라이브 사용 후 아래 방향키와 기본 공격을 순서대로 입력해 사용할 수도 있고 리프 토네이도, 거스트 다이브, 레전드리 스피어, 래쓰 오브 엔릴로 연계 가능하다.\n추가로 차지 드라이브와 하이킥 데몰리션의 데미지를 영구적으로 증가시키는 패시브 효과가 있다.\n\nMP 70 소비, 최대 8명의 적에게 360%의 데미지로 6번 공격\n사용 후 스피릿 점프 사용 초기화, 위 방향 점프는 초기화하지 않음\n재사용 대기시간 4초\n\n[패시브 효과 : 차지 드라이브, 하이킥 데몰리션 데미지 195%p 증가]";
            break;
        case 29:
            rep = "엔릴의 분노를 전방의 적에게 퍼붓는다. 연계 스킬을 사용하면 재사용 대기시간이 1초씩 줄어든다. 보스 몬스터 공격 시 일정 시간 공격 당한 보스와의 충돌을 무시하는 디버프를 건다. 차지 드라이브, 스트라이크 듀얼샷, 유니콘 스파이크, 레전드리 스피어, 라이트닝 엣지와 연계 가능하다.\n\nMP 200 소비, 전방의 10명의 적에게 515% 데미지로 10번 공격\n보스 몬스터 공격 시 보스와의 충돌을 무시하는 디버프가 60초 동안 발생\n\n재사용 대기시간 8초\n연계 스킬 사용 시 재사용 대기시간 1초 감소";
            break;
        case 9:
            rep = "";
            break;
