...
skwd.getskilldesc1Wild = function (skillcode){
    ...
    switch(skillcode){
        ...
        case 17:
            ...
            break;
        case 18:
            rep = "31초 동안 설치되어 5초 공격 후 2초 동안 장전을 반복, 쿨 8초";
            break;
        case 19:
            rep = "하단 참조";
            break;
...
skwd.getskilldesc3Wild = function (skillcode){
    ...
    switch(skillcode){
        case 0:
            rep = "영혼의 울부짖음으로 재규어 무리를 부른다. 아래 방향키와 함께 스킬을 사용하면 와일드헌터의 근처로 숫자는 줄어들지만 더 강한 재규어를 부를 수 있다.\n\nMP 1000 소비, 40초 동안 일정 범위 안에 포획한 재규어 중 현재 선택한 재규어를 제외한 재규어 7마리 소환, 소환된 재규어의 재규어 스킬 최종 데미지 반영률 90%, 지속 중 재규어 스킬로 일반 몬스터 공격 시 데미지 150% 증가 및 어나더 바이트 발생 확률 50% 증가\n아래 방향키와 함께 사용한 경우 재규어 3마리 소환, 소환된 재규어 스킬 최종 데미지 반영률 210%\n스킬을 다시 사용 혹은 아래 방향키와 함께 스킬을 다시 사용하면 자신이 있는 위치를 기준으로 재규어 재소환, 재소환은 3초마다 1번 사용가능\n\n재사용 대기시간 : 120초\n[패시브 효과 : 공격력 65 증가]";
            break;
        case 5:
