...
skk.getskilldesc3Kinesis = function (skillcode){
    ...
    switch(skillcode){
        case 0:
            ...
            break;
        case 1:
            rep = "염동력으로 움직일 수 있는 물질을 생성한다. 싸이킥 포스로 물질을 움직일 수 있다. 스킬을 다시 사용하면 물체를 키네시스 쪽으로 움직일 수 있지만 속도가 감소한다.\n아래 방향으로 싸이킥 포스를 사용했을 때는 이동 방향은 바뀌지만 이동 속도가 빨라지지 않는다. 공격 반사 상태의 적을 공격해도 피해를 입지 않는다.\n\n싸이킥 포인트 10칸 소비, 30초 동안 일정 주기마다 접촉한 최대 10명의 적을 1100%의 데미지로 5번 공격하는 물질 생성\n키네시스 쪽으로 물질을 움직일 때 현재 속도의 75%만 반영\n물질 소멸 시 최대 12명의 적을 1540%의 데미지로 12번 추가 공격\n\n재사용 대기시간 90초";
            break;
        case 2:
        ...
        case 19:
            ...
            break;
        case 22:
            rep = "염동력의 파동을 타고 하늘을 자유롭게 걷는다. 자신이 걸어가는 곳이 길이 되어 ↖, ↑, ↗ 3방향으로 이동할 수 있다. 스킬 레벨에 비례하여 지속 시간이 증가한다.\n다른 스킬 사용 중에도 사용할 수 있다.\n\n일정 시간 동안 비행, 일부 방향으로 이동 가능\n재사용 대기시간 4초\n\n다른 스킬 사용 중 사용하면 싸이킥 포인트 2칸 소비 및 재사용 대기시간 8초로 적용";
            break;
        case 27:
            ...
            break;
        case 29:
            rep = "시간의 간격보다 빠른 속도로 압축된 염동력을 전방에 쏟아낸다. 싸이킥 포인트를 모두 소모하면 염동력이 힘을 잃어, 일정 싸이킥 포인트 이상이 되었을 때 다시 쏟아내기 시작한다. 얼티메이트-B.P.M은 공격 반사 상태의 적을 공격해도 피해를 입지 않는다.\n스킬 사용 시 효과가 활성화되고 재사용 시 비활성화되는 온오프 스킬\n\n최대 3명의 적을 175% 데미지로 7번 공격, 1회 발동시 싸이킥 포인트 1칸 소비\n싸이킥 포인트 소진 시 공격 중지, 싸이킥 포인트 3이상 회복 시 공격 재개\n재사용 대기시간 0.5초\n\n[얼티메이트-메테리얼]의 데미지가 310%p만큼 증가\n[얼티메이트-딥 임팩트]의 데미지가 270%p만큼 증가\n[얼티메이트-트레인]의 데미지가 100%p만큼 증가";
            break;
        case 30:
            rep = "더욱 강력하고 넓은 싸이킥 영역을 생성한다. 생성 시 1회에 한해 적을 끌어 당긴다. ↑+[싸이킥 포스]로 시전이 가능하다. 범위 내 몬스터의 방어율과 이동속도를 감소시키고 싸이킥 공격의 최종 데미지를 증가시킨다.\n\n최초 1회 5명의 적에게 1000% 데미지로 4번 공격\n영역 내 적 이동속도 30% 감소, 키네시스의 공격에 방어율 15% 추가 무시 및 받는 최종 데미지가 15% 증가하는 디버프 부여\n30초 동안 지속, 영역에 들어왔다가 나간 적에게도 영역의 남은 지속 시간만큼 디버프 적용\n[스킬 사용 시 싸이킥 포인트 1칸 충전]\n\n재사용 대기시간 30초";
            break;
        case 31:
            rep = "정신 파동을 일으켜 순간적으로 적들의 정신을 붕괴시킨다. 영구적으로 최종 데미지가 증가한다. 컴뱃 오더스가 적용되지 않는다.\n\n최종 데미지 40% 증가";
            break;
        case 39:
