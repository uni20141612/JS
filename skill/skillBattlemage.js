...
skbm.getskilldesc1Battlemage = function (skillcode){
    ...
    switch(skillcode){
        ...
        case 17:
            ...
            break;
        case 18:
            rep = "텔레포트 Ver.2 이동 거리 좌우 65, 상하 30 증가";
            break;
        case 19:
            rep = "최종 데미지 25%";
            break;
        ...
        case 29:
            ...
            break;
        case 30:
            rep = "MP 80 소비, 8명의 적에게 650% 데미지로 7번 공격.";
            break;
        case 31:
            rep = "";
            break;
...
skbm.getskilldesc2Battlemage = function (skillcode){
    ...
    switch(skillcode){
        ...
        case 9:
            ...
            break;
        case 10:
            rep = "[패시브 효과 : 데스 컨트랙트의 데미지 80%p 증가, 중첩되지 않음]";
            break;
        case 11:
            rep = "블로우류 스킬과 연계가능";
            break;
        ...
        case 26:
            ...
            break;
        case 27:
            rep = "몬스터 방어율 무시 30% 증가";
            break;
        case 28:
            rep = "최종 데스 강화 패시브";
            break;
        case 29:
            rep = "블로우, 데스 강화 액티브 버프";
            break;
        case 30:
            rep = "재사용 대기시간 13초";
            break;
...
skbm.getskilldesc3Battlemage = function (skillcode){
    ...
    switch(skillcode){        
        ...
        case 7:
            ...
            break;
        case 8:
            rep = "흑마술의 비급을 사용해 사신과 계약한다. 사신은 자신을 따라다니며, 일정 수 이상의 영혼을 수집하면 본 모습을 드러내 넓은 범위를 공격해준다. 사신의 공격은 공격 반사 상태의 적에게도 피해를 입히고 자신의 레벨에 비례한 공격력을 가진다.\n\n적을 10명 해치운 뒤 그 다음 공격을 할 때 최대 7명의 적에게 185%로 7번 공격\n보스에게 2번 공격하면 적을 1명 해치운 것으로 취급\n발동 후 9초 동안 다시 발동하지 않음";
            break;
        case 12:
            rep = "자신의 몸에서 붉은 빛의 오라를 방출하여 주위의 파티원들이 적에게 입힌 데미지의 일부를 HP로 흡수시켜주는 능력을 가지게 한다. 한 번에 캐릭터 최대 HP의 20%를 초과하여 회복할 수 없다. 스킬을 한 번 더 사용해서 오라를 해제할 수 있으며, 다른 종류의 오라를 사용하면 해제된다. 추가로 적을 해치울 때마다 적의 영혼을 제물로 최대 HP의 일정비율을 회복하는 능력을 영구히 가진다.\n\n초당 MP 28 소비, 준 데미지의 1%만큼 HP 회복, 한 번 회복 후 5초 동안 재회복 불가, 배틀메이지가 준 데미지는 드레인 오라 범위 안의 파티원의 HP도 함께 회복시킴\n\n재사용 대기시간 3초\n[패시브 효과 : 4초마다 최대 HP의 2% + 1000 회복, 적을 해치울 때마다 최대 HP의 1% 회복]";
            break;
        case 13:
        ...
        case 16:
            ...
            break;
        case 17:
            rep = "텔레포트를 사용하면 출발점과 도착점 사이를 검은 번개로 공격하며 맞은 적에게 검은 징표를 새긴다.\n다크 라이트닝을 배우면 다크 체인, 배틀 스퍼트 사용 시에도 검은 징표가 새겨지며 징표를 새기는 스킬과 소환수로는 징표가 발동되지 않는다. 또한 옐로우 오라 사용 중 텔레포트 Ver.2의 이동거리가 증가하고 스킬UI에서 옐로우 오라를 우클릭하여 기능을 온오프 할 수 있다.\n\n텔레포트 사용 시 MP 15 추가 소비, 텔레포트 시작점과 도착점 사이 최대 8명의 적을 100%의 데미지로 4번 공격, 일반 몬스터 공격 시 데미지 150%p 증가, 공격을 받은 적은 15초 동안 검은 징표가 새겨짐\n\n검은 징표가 새겨진 적을 공격 시 징표가 발동하여 400%의 데미지로 4번 공격, 징표 발동 공격은 보스 몬스터 공격 시 데미지 20% 증가, 징표는 15번 공격 시 소비";
            break;
        case 21:
            rep = "사신과 계약을 더욱 강화한다.\n\n적을 6명 해치운 뒤 그 다음 공격을 할 때 최대 8명의 적에게 325%로 10번 공격하는 것으로 강화\n발동 후 6초 동안 다시 발동하지 않음.";
            break;
        case 22:
            rep = "죽음의 번개로 최대 15마리의 적을 동시에 공격하고 일정 시간 동안 기절시킨다. 다크 제네시스는 공격 반사 상태의 적을 공격해도 피해를 입지 않는다. 추가로 다크 제네시스 스킬을 배우면 다크 체인, 배틀 스퍼트, 다크 라이트닝의 데미지가 영구적으로 증가하고 적을 공격하면 단일 적에게 번개 공격을 하여 피해를 입힌다.\n\nMP 200 소비, 최대 15명의 적에게 520% 데미지로 8번 공격하고 4초 동안 기절시킴\n\n재사용 대기시간 30초\n\n[패시브 효과 : [파이널 어택류 스킬] 직접 공격하는 공격 스킬 사용 시 단일 적에게 60% 확률로 220% 데미지의 번개로 공격 및 다크 체인, 배틀 스퍼트, 다크 라이트닝의 데미지 60% 증가]";
            break;
        case 23:
            rep = "자신의 몸에서 신비한 힘을 가진 검은 오라를 방출하여 파티원들의 데미지를 증가시킨다. 스킬키를 한 번 더 사용해서 오라를 해제할 수 있으며, 다른 종류의 오라를 사용하면 해제된다. 추가로 마력이 영구히 증가하며 컴뱃 오더스가 적용되지 않는다.\n\n초당 MP 16 소비, 데미지 20% 증가.\n\n재사용 대기시간 3초\n\n[패시브 효과 : 마력 15% 증가]";
            break;
        case 24:
            rep = "자신의 몸에서 불길한 오라를 방출하여 적을 약화시킨다. 스킬키를 한 번 더 사용해서 오라를 해제할 수 있으며, 다른 종류의 오라를 사용하면 해제된다. 디버프 오라는 적에게 적용되는데 일정 시간이 필요하지만 오라를 해제하고서도 일정 시간 효과가 적용된다. 컴뱃 오더스가 적용되지 않는다.\n\n초당 MP 26 소비, 적의 방어율 20% 감소. 적용되는 데 2초 필요하나 오라 해제 후에도 60초 동안 적용.\n\n재사용 대기시간 3초";
            break;
        case 25:
            rep = "적을 집중 공격할 수 있는 상태가 되며 데미지가 비약적으로 증가한다. 또한 영구적으로 신체를 활성화 하여 자신의 최대 HP와 MP를 영구히 증가시킨다.\n\nMP 50 소비, 데미지 25% 증가, 크리티컬 확률 20% 증가, 크리티컬 데미지 10% 증가\n일부 소환수를 제외한 자신의 모든 공격 스킬 최대 공격 가능한 몬스터 수 1/3로 감소, 1명만 공격 가능한 스킬이 아닌 스킬의 최소치 2명\n데스, 유니온 오라로 강화된 사신의 낫, 블랙 매직 알터, 그림 리퍼는 최대 공격 가능한 몬스터 수 감소 제외\n\n[패시브 효과 : 최대 HP 30%, 최대 MP 30% 증가]";
            break;
        case 26:
            rep = "자신을 중심으로 마법의 보호막을 생성하여 파티원들을 보호한다. 보호막 안에서는 적의 공격에 데미지를 받지 않으며, 고정 피해를 입히는 공격에 피격 시 피해를 감소시킨다.\n\nMP 150 소비, 지속 시간 40초, 최대 HP의 일정 비율로 피해를 입히는 공격에 피격 시 10% 피해 감소\n\n재사용 대기시간 180초\n\n---무효화되는 패턴 목록---\n아카이럼과 카오스 반반의 즉사기\n파풀라투스의 시간의 틈/레이저 교차\n스우의 보라 구체/전깃줄\n윌의 모니터 브레이크/2페이즈 양 끝 거미줄\n세렌의 화염 장막";
            break;
        case 28:
            rep = "사신과 계약을 더욱 강화한다.\n\n적을 6명 해치운 뒤 그 다음 공격을 할 때 최대 10명의 적에게 500%로 12번 공격하는 것으로 강화. 발동 후 5초 동안 다시 발동하지 않음.";
            break;
        case 29:
            rep = "사신의 주인이 되어 그 능력을 마음껏 부린다.\n\nMP 300 소비, 30초 동안 블로우 스킬의 공격 횟수 1번 증가\n사신이 수집해야 하는 영혼의 수 1명으로 감소, 데스의 최종 데미지 50% 증가, 데스를 제외한 어둠 속성 공격이 적중될 때마다 데스의 재발동 대기시간 0.5초씩 감소\n\n재사용 대기시간 200초";
            break;
        case 30:
            rep = "";
            break;
