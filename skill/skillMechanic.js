...
skm.getskilldesc1Mechanic = function (skillcode){
    ...
    switch(skillcode){
        ...
        case 21:
            ...
            break;
        case 22:
            rep = "1차 타격 : 1명의 적을 285%의 데미지로 6번 공격";
            break;
        case 23:
            rep = "하단 참조";
            break;
...
skm.getskilldesc3Mechanic = function (skillcode){
    ...
    switch(skillcode){
        ...
        case 16:
            ...
            break;
        case 17:
            rep = "3개의 자기장 코일을 설치하여 지속적으로 적을 공격할 수 있는 <마그네틱 필드>를 생성한다. 자기장 코일은 지속시간이 경과하면 스스로 폭발한다.\n자기장 코일이 1개도 설치되어 있지 않고 3개가 한번에 설치되기에 충분한 공간이 있다면 아래 방향키와 함께 사용하여 3개의 자기장 코일을 한 번에 설치할 수 있다.\n마그네틱 필드는 공격 반사 상태의 적을 공격해도 피해를 입지 않고 로봇 마스터리와 마그네틱 필드-퍼시스트를 제외한 소환 지속시간 증가의 효과를 받지 않는다.\n\nMP 75 소비, 자기장 코일을 3개 설치했을 경우 60초 동안 마그네틱 필드 생성, 범위 내 10명의 적들에게 200% 데미지와 함께 기절 효과 적용, 지속시간 경과 시 550%의 데미지로 자폭\n\n재사용 대기시간 160초";
            break;
        case 23:
            rep = "거대로봇 <타이탄>을 소환하여 보다 많은 적이 있는 곳에 미사일 폭격을 가한다. 타이탄을 소환하면 일정 시간 동안 활성화 게이지가 나타나는데, 일반 공격 키를 연타하여 게이지를 충전하면 데미지를 최대 2배까지 증폭시킬 수 있다. 스킬 사용 중에는 받는 피해가 감소한다.\n\nMP 400 소비, 최대 15명의 적에게 500% 데미지로 8번 공격. 타이탄에게 근접한 적에게 초당 1300%의 데미지로 2번 공격. 사용 중 최대 HP의 일정 비율로 피해를 입히는 공격을 포함한 피격 데미지 50% 감소.\n\n재사용 대기시간 30초";
            break;
        case 25:
            rep = "일정 주기로 토이 로봇을 소환하는 로봇 팩토리 <RM1>을 설치한다. 소환된 토이 로봇들은 RM1 주변에서 적을 발견하거나 일정 시간이 경과하면 자폭하여 데미지를 주며, 로봇 팩토리 또한 지속시간이 경과하면 스스로 폭발한다.\n<로봇 팩토리 : RM1>은 공격 반사 상태의 적을 공격해도 피해를 입지 않는다.\n\nMP 100 소비, 30초 동안 로봇 팩토리 설치. 3초마다 500% 데미지의 토이 로봇 3개 소환, 지속시간 경과 시 1000%의 데미지로 자폭\n\n재사용 대기시간 60초";
            break;
        case 27:
