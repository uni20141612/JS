...
skr.getskilldesc1Rara = function (skillcode){
    ...
    switch(skillcode){
        ...
        case 8:
            ...
            break;
        case 9:
            rep = "피격 시 MP 40을 소비하여 피격 데미지 50% 감소";
            break;
        case 10:
        ...
        case 21:
            ...
            break;
        case 22:
            rep = "용맥 소모 시 최대 HP와 최대 MP 1% 회복 후";
            break;
        case 23:
...
skr.getskilldesc2Rara = function (skillcode){
    ...
    switch(skillcode){
        ...
        case 28:
            ...
            break;
        case 29:
            rep = "쿨 3초, 속성 변환 성공 시 스킬 쿨 7초 증가";
            break;
        case 30:
            rep = "분출 스킬 강화 패시브";
            break;
...
skr.getskilldesc3Rara = function (skillcode){
    ...
    switch(skillcode){
        ...
        case 12:
            ...
            break;
        case 13:
            rep = "여러 강의 토지령이 힘을 합쳐 거대화한 후 일정 간격으로 너울을 일으켜 주변을 공격한다.\n\n16초 동안 강의 토지령 유지, 강의 토지령이 일으킨 너울은 최대 6명의 적을 215% 데미지로 4번 공격, 너울은 최대 8번 발생";
            break;
        case 14:
            rep = "바람의 토지령이 일정 시간마다 크게 숨을 내뱉는다. 바람의 토지령의 숨결로 돌개바람이 생성되어 주변의 적을 지속 공격한다.\n\n16초 동안 바람의 토지령 유지, 바람의 토지령이 만든 돌개바람은 최대 6명의 적을 63% 데미지로 5번 지속 공격, 돌개바람은 최대 5번 생성";
            break;
        case 15:
            rep = "여러 해의 토지령이 힘을 합쳐 무서운 거인이 된 후 흩어져 내리며 주변을 용암이 솟아오르는 지역으로 만든다. 용암 지역에서 일정 간격으로 화산탄이 생성되어 용암 지역 위의 적을 공격한다. 화산탄은 범위 내의 보스 몬스터를 우선하여 추적하며, 보스 몬스터가 여럿일 경우 최대 HP가 가장 높은 몬스터를 우선하여 추적한다.\n\n거인이 솟아오르며 최대 6명의 적을 120% 데미지로 6번 공격, 공격 후 16초 동안 용암 지역 형성, 용암 지역은 1초마다 최대 10명의 적을 85% 데미지로 1번 공격\n일정 간격마다 적을 75% 데미지로 3번 공격하는 화산탄이 4개 생성, 화산탄은 최대 5번 생성되며 하나의 적이 여러 개의 화산탄에 맞을 경우 두번째 화산탄부터 최종 데미지 10% 감소, 화산탄에 맞은 적은 8초 동안 60%의 도트 데미지";
            break;
        case 16:
            ...
            break;
        case 17:
            rep = "용맥에 담긴 강의 힘이 발현하며 흐르는 강을 타고 이동할 수 있게 된다. 스킬 사용 후 일정 시간 안에 스킬을 재사용하면 강줄기의 도착점을 지정할 수 있다. 도착점이 지정되면 시작점과 도착점 사이에 강줄기가 지속적으로 흐르며 라라가 시작점에서 위 방향키를 입력하면 강을 타고 이동할 수 있다. 도착점은 포탈이 있는 위치 및 일부 위치에는 지정할 수 없고 사냥 맵이 아닌 일부 맵에서는 사용할 수 없다. 발현 스킬은 자유로운 용맥을 가장 우선하며 자유로운 용맥이 없다면 가까운 용맥을 우선 소모한다. 시전 동작 중 스탠스 100%가 적용된다. 강을 타고 이동 중 발현 : 강줄기 흐르는 곳을 제외한 용맥을 소모하는 스킬을 1회 사용할 수 있고 소모할 수 있는 범위가 증가하며 이동한 거리에 비례해 용맥을 소모하는 스킬을 추가로 사용할 수 있다. 해당 스킬은 버프 프리저의 효과를 받지 않는다.\n\nMP 80 소비, 스킬 사용 시 20초 동안 도착점 지정 가능, 도착점 지정 시 90초 동안 시작점과 도착점을 흐르는 강줄기 생성, 시작점에서 위 방향키 입력 시 강줄기를 타고 이동 가능\n재사용 대기시간 60초\n\n[패시브 효과 : 너울이는 강의 데미지 120%p 증가]";
            break;
        case 18:
            rep = "용맥에 담긴 바람의 힘이 발현하여 상승 기류가 지속해서 발생하는 영역이 생성된다. 공격 속도와 이동속도, 점프력이 일정 시간 동안 증가되는 버프를 얻을 수 있고 중앙의 빛무리에서 위 방향키를 입력하면 바람이 밀어주는 그네를 타고 영역 안에서 부유할 수 있다. 발현 스킬은 자유로운 용맥을 가장 우선하며 자유로운 용맥이 없다면 가까운 용맥을 우선 소모한다. 시전 동작 중 스탠스 100%가 적용된다.\n\nMP 80 소비, 30초 동안 지속되는 바람 그네 설치\n영역에 들어오면 공격 속도 1단계, 이동 속도 20, 점프력 20만큼 증가하는 버프 지급\n해당 버프는 자신에게는 120초, 파티원에게는 20초 동안 지속\n중앙의 빛무리에서 위 방향키 입력 시 공중으로 떠오르며 최대 4.5초 동안 부유 가능\n재사용 대기시간 60초\n\n[패시브 효과 : 돌개바람의 데미지 35%p 증가]";
            break;
        case 19:
        ...
        case 20:
            ...
            break;
        case 21:
            rep = "용맥의 자취를 쫓는다. 일정 간격으로 몸에 축적되는 땅의 기운을 사용하여 근처에 있는 용맥의 위치로 순식간에 이동하고 이동 후 용맥 주위의 적을 공격한다. 8방향으로 사용할 수 있어 방향키와 함께 사용하면 해당 방향의 용맥을 우선적으로 찾고, 범위 내의 자유로운 용맥을 가장 우선하며 없다면 가까운 용맥으로 이동한다. 현재 위치와 매우 가까운 용맥으로는 이동하지 않는다.\n\nMP 65 소비, 땅의 기운 1개 소비, 최대 5명의 적을 500% 데미지로 2번 공격\n땅의 기운은 6초마다 1개씩 준비되며 최대 3개까지 저장 가능\n재사용 대기시간 1초";
            break;
        case 24:
        ...
        case 25:
            ...
            break;
        case 26:
            rep = "정기 뿌리기 적중 시 장난꾸러기 강의 토지령이 강 웅덩이와 함께 등장하여 적에게 물벼락을 뿌려 공격한다.\n\n90초 동안 정기 뿌리기 적중 시 강의 토지령이 나타나 최대 6명의 적을 550% 데미지로 6번 공격\n재발동 대기시간 2.5초";
            break;
        case 27:
            rep = "정기 뿌리기 적중 시 바람의 토지령이 조용히 나타나 적을 지속 공격하는 소소리 바람을 일으킨다.\n\n90초 동안 자신이 정기 뿌리기 적중 시 적중한 적 중 하나의 위치에 소소리 바람이 발생, 소소리 바람은 최대 6명의 적을 237% 데미지로 2번 지속 공격\n재발동 대기시간 2.5초";
            break;
        case 28:
            rep = "정기 뿌리기 적중 시 해의 토지령이 모여 뜨거운 햇빛이 응축된 화염구를 뿜어내 적을 추적 공격한다. 화염구는 범위 내의 보스 몬스터를 우선하여 추적하며, 보스 몬스터가 여럿일 경우 최대 HP가 가장 높은 몬스터를 우선하여 추적한다.\n\n90초 동안 정기 뿌리기 적중 시 해의 토지령이 나타나 화염구 5개를 뿜어 공격, 화염구는 주변의 적을 추적하여 220% 데미지로 6번 공격\n재발동 대기시간 2.5초";
            break;
        case 30:
            rep = "자연을 즐기고 하나가 되어 자연의 힘을 더욱 잘 다룰 수 있게 된다. 분출 스킬 사용 시 용맥에서 더 많은 잠재력을 끌어낼 수 있어 분출 스킬의 위력이 증가한다.\n\n분출 스킬 유지 시간 18초로 증가\n\n너울이는 강 : 너울의 최대 공격 가능한 몬스터 수 2 증가, 공격 횟수 5번으로 증가 및 데미지 295%p 증가, 일정 간격으로 큰 너울이 발생하여 최대 10명의 적을 638% 데미지로 8번 공격, 일반 너울과 큰 너울 합하여 너울 총 9번 발생\n돌개바람 : 돌개바람의 범위 증가, 최대 공격 가능한 몬스터 수 4 증가 및 데미지 218%p로 증가\n해돋이 우물 : 거인의 최대 공격 가능한 몬스터 수 4 증가 및 데미지 337%p 증가, 용암 지역 생성 범위 증가 및 용암 지역의 데미지 190%p 증가, 화산탄의 데미지 233%p 증가, 화산탄에 맞은 적에 적용되는 도트 데미지 50%p 증가, 화산탄이 더 자주 생성되며 한 번에 5개씩 최대 5번 생성";
            break;
        case 32:
        ...
        case 33:
            ...
            break;
        case 36:
            rep = "어떤 힘이든 머금을 수 있는 자유로운 용맥을 생성한다. 생성된 자유로운 용맥에 분출, 흡수, 발현 스킬을 사용하면 원하는 속성으로 용맥의 힘을 끌어낼 수 있다. 자유로운 용맥은 용맥 읽기가 활성화된 상태에서만 사용 가능하고 재사용 대기시간 초기화의 효과를 받지 않으며 시전 동작 중 스탠스 100%가 적용된다. 자유로운 용맥을 생성 중 용맥 분출, 용맥 흡수를 사용하면 자유로운 용맥의 시전 동작을 하지 않고 분출, 흡수할 속성을 선택할 수 있다.\n\nMP 500 소비, 전방에 자유로운 용맥 생성\n자유로운 용맥에 용맥 분출, 용맥 흡수 사용 시 2초 동안 끌어내고자 하는 속성 선택 가능\n자유로운 용맥은 한 맵에 최대 3개까지 존재 가능\n자유로운 용맥은 10초마다 1개씩 준비되며 최대 3개까지 소지 가능\n\n재사용 대기시간 1.5초";
            break;
        case 37:
